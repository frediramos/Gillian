...
module type ObjectIntf = sig
  ...
  val create : unit -> t
  val clone : t -> t
  val set : t -> Expr.t -> Expr.t -> t
  val get : t -> vt -> vt -> Type_env.t -> (vt * vt) list
  val get_fields : t -> vt list
  val delete : t -> vt -> t
  val to_list : t -> (vt * vt) list
  val from_list : (vt * vt) list -> t
  val to_sfvl : t -> SFVL.t
  val from_sfvl : SFVL.t -> t
  val pp : Format.formatter -> t -> unit
end

module MakeHeap (O : ObjectIntf) = struct
  type ot = O.t [@@deriving yojson]
  type init_data = unit
  ...
  let lvars (heap : t) : Containers.SS.t =
    ...
    let lvars_fvl =
      Hashtbl.fold
        (fun _ fvl ac -> Var.Set.union (to_ss @@ O.to_list fvl) ac)
  ...
  let merge_loc (heap : t) (new_loc : string) (old_loc : string) : unit =
    let empty = O.create () in
    let fvl, dom, met =
      match Hashtbl.mem heap.fvl new_loc with
      | true ->
          (* Merge field-value lists *)
          let ofl =
            Option.value ~default:empty (Hashtbl.find_opt heap.fvl old_loc)
          in
          let nfl =
            Option.value ~default:empty (Hashtbl.find_opt heap.fvl new_loc)
          in
          let fvl = SFVL.union (O.to_sfvl nfl) (O.to_sfvl ofl) in
  ...
  let substitution_in_place (subst : SSubst.t) (heap : t) : unit =
    ...
    if not (SSubst.domain subst None = Expr.Set.empty) then (
      ...
      Hashtbl.iter
        (fun loc obj ->
          let fvl = O.to_sfvl obj in
          let fvl_sub = SFVL.substitution subst true fvl in
          Hashtbl.replace heap.fvl loc (O.from_sfvl fvl_sub))
        heap.fvl;
  ...
  let pp_ot fmt (o : ot) : unit = O.pp fmt o
  ...
  let object_set o prop v = O.set o prop v
  let object_get o prop pc gamma = O.get o prop pc gamma
  let object_to_list o = O.to_list o
  let object_get_fields o = O.get_fields o
  ...
  let set_cell
    ...
    let obj =
      ...
      | None -> object_set (O.create ()) prop v
      | Some o -> object_set o prop v
    in
  ...
  let remove_cell
    ...
    let f (loc_name : string) : unit =
      Option.fold
        ~some:(fun ((obj, dom), mtdt) ->
          let obj' = object_set obj prop none in
  ...
  let get_partial_domain
    ...
    let f loc_name =
      ...
      | Some ((fv_list, Some dom), mtdt) -> (
          let pairs = object_to_list fv_list in
  ...
  let get_full_domain (heap : t) (pfs : PFS.t) (gamma : Type_env.t) (loc : vt) :
    ...
    let f loc_name =
      ...
      | Some ((fv_list, Some dom), _) ->
          let props = object_get_fields fv_list in
          let pairs = object_to_list fv_list in
  ...
  let sorted_locs_with_vals (heap : t) =
    ...
    List.map
      (fun loc ->
        ...
        let obj' = O.to_sfvl obj in
        (loc, ((obj', dom), met)))
      sorted_locs

  let get (heap : t) (loc : string) =
    Option.map
      (fun sfvl -> ((O.to_sfvl sfvl, get_dom heap loc), get_met heap loc))
      (get_fvl heap loc)
