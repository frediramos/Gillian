...
module LoggingObj : ObjectIntf = struct
  type ot = ot' list
  and ot' = Rec of (vt * vt) [@@deriving yojson]
  ...
  let set (obj : ot) (field : vt) (value : vt) : ot = Rec (field, value) :: obj
  ...
  and get_vals (obj : ot) (prop : vt) (pc : vt) (gamma : Type_env.t) :
    ...
    | r :: obj' ->
        ...
        else if is_sat [ pc; pi1 ] gamma then
...
module M = struct
  module Heap = MakeHeap (LoggingObj)
  include Heap
